...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(document).on('submit','#myForm',function(e){
			...
			$.ajax({
				url:'<?php echo base_url() ?>crud/newSlider',
				type:'post',
				...
				success:function(response){
					...
					if(response == '"done"'){
						...
						swal('','Data slider berhasil dimasukkan','success').then(function(){
		...
		$(document).on('submit','#imgForm',function(e){
			...
			$.ajax({
				url:'<?php echo base_url() ?>crud/editSliderImg',
				type:'post',
				data: new FormData(this),
				contentType:false,
				processData:false,
				success:function(response){
					...
					if(response == '"done"'){
						...
						swal('','Foto slider berhasil diubah','success').then(function(){
		...
		$(document).on('submit','#editForm',function(e){
			...
			$.ajax({
				url:'<?php echo base_url() ?>crud/editSliderInfo',
				type:'post',
				data: new FormData(this),
				contentType:false,
				processData:false,
				success:function(response){
					...
					if(response == '"done"'){
						...
						swal('','Info slider berhasil diubah','success').then(function(){
	...
	function loadData(){
		...
		$.ajax({
			url:'<?php echo base_url() ?>data/slider',
			type:'get',
			dataType:'json',
	...
	function formCrud(id){
		...
		if(id != 'baru'){
			url1 = url1+'form/sliderEdit/'+id;
		}else{
			url1 = url1+'form/newSlider';
		}
	...
	function sliderImg(id){
		...
		$.ajax({
			url:'<?php echo base_url() ?>form/sliderImg/'+id,
			type:'get',
			dataType:'json',
			success:function(data){
				$('#form').modal('show');
	...
	function show(id){
		...
		$.confirm({
			...
			content:'url:<?php echo base_url() ?>data/sliderInfo/'+id,
			columnClass:'large',
			buttons:{
				yup:{
					text:'Iya',
					...
					action:function(){
						...
						$.ajax({
							url:'<?php echo base_url() ?>crud/sliderstat/Show/'+id,
							type:'post',
							contentType:false,
							processData:false,
							success:function(response){
								...
								if(response == '"done"'){
									...
									swal('','Slider ditampilkan','success').then(function(){
			...
			buttons:{
				yup:{
					...
					action:function(){
						...
						$.ajax({
							url:'<?php echo base_url() ?>crud/sliderstat/Hidden/'+id,
							type:'post',
							contentType:false,
							processData:false,
							success:function(response){
								...
								if(response == '"done"'){
									...
									swal('','Slider disembunyikan','success').then(function(){
